...
- **Anonymous Telemetry** - Every ~15 minutes (independent of the version-check interval), sends the app version, update channel (`stable`/`beta`), OS name and version, CPU architecture, and connected-appliance model id(s) and count to a self-hosted [Aptabase](https://aptabase.com) instance. It also sends a stable per-install id — a one-way SHA-256 hash of your Electrolux username (never the username itself or any credential) — so the same install can be counted over a rolling 24h window without revealing who you are. Aptabase additionally derives an approximate country from the request IP. Opt out by setting `E2M_TELEMETRY_ENABLED=false`. This is a personal project maintained in my free time, and knowing the active user count helps me gauge how much effort to invest in it.
...
> For detailed descriptions and reasoning behind each option, see the comments in [`config.example.yml`](./config.example.yml).
