...
- **`Orchestrator` (`src/orchestrator.ts`)** — runtime brain. Polls each appliance on `refreshInterval`, periodic `discoverAppliances()` for added/removed devices, subscribes command topics, tracks every interval/subscription for clean `shutdown()`. Only component besides `Cache` holding mutable runtime state.
...
- **`appliances/`** — plugin layer. `BaseAppliance` abstract; concrete models registered in `ApplianceFactory.create()` (currently only `Comfort600`; unknown portable AC models fall back). Each appliance owns `generateAutoDiscoveryConfig()`, `normalizeState()`, `transformMqttCommandToApi()` — last sole authority for API-specific denormalization (`medium`→`MIDDLE`, `fan_only`→`FANONLY`).
...
- **`cache.ts`** — `tiny-lru` state cache (24h TTL). Orchestrator diffs incoming API state vs cache to decide what to publish to MQTT.
- **`version-checker.ts`** — periodic GitLab release check + ntfy.sh push + anon telemetry POST to `telemetry-backend/` service. User hash = HMAC-SHA-256 with install-unique salt (hashed MQTT URL suffix + username prefix + country code).
- **`health.ts`** — periodically touches file for Docker `HEALTHCHECK`. Best-effort writes: prod `dhi.io/node` base image read-only, failures log once, stop retrying.

`telemetry-backend/` standalone pnpm package (own `package.json`, `Dockerfile`, Vitest suite). HTTP surface in `src/app.ts` behind `RedisLike` interface (testable via `tests/fake-redis.ts`); `src/index.ts` thin bootstrap adapting real node-redis v5 client. Rate limiting runs before payload validation.

API type unions in `src/types.d.ts` and `src/types/normalized.ts` kept in sync with E2E fixtures under `tests/e2e/snapshots/<model>/`. `.claude/agents/engineer.md` per-change-type file checklists; `.claude/skills/{audit,maintain}/` define `/audit` + `/maintain` slash-command workflows.
