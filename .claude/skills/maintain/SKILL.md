...
3. **Apply updates**
   ...
   - **Node LTS major or Alpine bumped** → edit `mise.toml` (`[tools] node` or `[env] ALPINE_VERSION`), then run `pnpm sync:versions`. The script propagates the value to all derived files (`.nvmrc`; both `engines.node`; both `devDependencies['@types/node']`; `docker/Dockerfile` + `Dockerfile.local` `ARG NODE_VERSION`; the compose `NODE_VERSION:-<major>` defaults; `telemetry-backend/Dockerfile`; `telemetry-backend/docker-compose.yml`; the `.gitlab/ci/01_init.yml` alpine literal). A Node major bump therefore also moves `@types/node` — run `pnpm install` in both packages afterwards so the lockfiles re-resolve. Never hand-edit a derived file. Never change the prod Dockerfile's hardened `dhi.io/node` base. CI `versions in sync` enforces no drift reaches the repo.
