...
Single long-running HTTP service (Node built-in `http`, no Express/Redis): reads Aptabase ClickHouse behind `ClickHouseLike`, writes badge SVGs + `telemetry.json` (incl. trailing-window `peaks` sampled by `peaks.ts` and persisted in `peaks-history.json`) to `OUTPUT_DIR` (served statically by the reverse proxy; `GET /telemetry` serves the JSON in-memory), forwards legacy `POST /telemetry` via `AptabaseForwarder`. Regenerates every `BADGE_INTERVAL_SECONDS`.
