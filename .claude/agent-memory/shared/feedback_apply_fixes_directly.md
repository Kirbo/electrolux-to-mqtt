...
**How to apply:** Diagnose → edit → prove it works → report. Applying to the working tree is not the same as committing — commits still need their own trigger (see [[feedback_memory_before_commit]]). This refines [[feedback_delegate_by_default]]: "ask first" is for ambiguous/large work, not for a clear fix the user just requested.
