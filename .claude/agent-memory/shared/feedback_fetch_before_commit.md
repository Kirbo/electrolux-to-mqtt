...
**How to apply:** Since 2026-08-17 this is AUTOMATED: a SessionStart hook (`.claude/hooks/check-origin-freshness.sh`, one fetch per 6h via a `.git/` marker) warns in-context when the branch is behind, and `require-branch-sync.sh` hard-blocks `git commit` on a behind branch. Do not fetch manually before every task — act on the hook warnings instead: if one fired, tell the user and sync before starting the work.
